...
def add_parchment_background(c, doc):
    ...
    try:
        ...
        logger.warning(
            "Unable to draw parchment background from %s: %s",
            background_path,
            e,
        )
...
async def pdf_generator(transcriptions, logo_path=None):
    ...
    try:
        doc.build(
            elements,
            onFirstPage=add_parchment_background,
            onLaterPages=add_parchment_background,
        )
    except Exception as e:
        logger.exception("Failed to build PDF document: %s", e)
        raise
